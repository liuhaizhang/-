...
def xml2data(xmlname):
    ...
    filename = xmlname.replace("_ai.xml", ".tiff")
    ...
    xml = minidom.parse(path.replace(".tiff", "_ai.xml"))
    ...
    for i in range(len(annos)):
        ...
        sql = "insert into t_slide_label(slide_id,label_info,slide_file_name,creator,create_time,update_time)" \
              " values ({},'{}','{}', null,'{}','{}')".format(slide_id, label_info, filename,
                                                             dt, dt)
        cursor.execute(sql)
        connection.commit()
    connection.close()
